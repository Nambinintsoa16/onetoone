...
if($uploadOk == 0){
   ...
   //if(move_uploaded_file($_FILES['file']['tmp_name'],$location)){
      $sql="INSERT INTO `clientTr`(`id`, `codeClient`, `Nom`, `Prenom`, `codevp`, `ville`, `sexe`, `dateNaiss`, `contact`) VALUES (?,?,?,?,?,?,?,?,?)";
      $main->query($sql,array(null,$codeClient,$nom,$prenom,$_SESSION['matricule'],$ville,$sexe,"2019-11-20",$contact));
      ...
      if(($date>=date("08:00:00") AND $date<date("11:30:00")) OR  ($date>=date("14:00:00") AND $date<date("17:30:00"))  ){
         $main->historique(array(null,date("Y-m-d"),$date,$activites, $_SESSION['matricule'],25));
             $nbPoint=$main->testPoint($_SESSION['matricule']);
             //pour coach
                    $coach=$main->getCoachDuCom($_SESSION['matricule']);
                    $main->historique(array(null,date("Y-m-d"),$date,'coach', $coach['coach'],7));
           ...
           //pour coach
                    $coach=$main->getCoachDuCom($_SESSION['matricule']);
                    $main->historique(array(null,date("Y-m-d"),$date,'coach', $coach['coach'],3));
                    $id_coach=$main->getIdStory_coach($coach['coach'],date("Y-m-d"),$date);
                    $nbPoint_coach=$main->testPoint($coach['coach']);
                //Fin coach
           if($nbPoint){
      ...
      $activite="vente";
        ...
        foreach ($content_produit as $key=>$content_produit) {
            $sql="INSERT INTO `vente`(`idVente`,`codeClient`,`quantite`, `lieu`, `date`, `idVP`, `heure`,`codeproduit`, `ville`, `quartier`,`id_equipe`,`id_historique_coach`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
                $main->query($sql,array(null,$codeClient,$content_quantite[$key],$_SESSION['idMission'],$dateDB,$_SESSION['matricule'],$date,$content_produit,$_GET['ville'],$_GET['quartier'],$equipe['designationEquipe'],$id_coach));
        }
