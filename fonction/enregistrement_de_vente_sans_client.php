...
$Date=$dt->format("Y/m");
if (isset($_POST['content_produit']) AND isset($_POST['content_quantite']) AND isset($_POST['ville']) AND isset($_POST['quartier']) ) {
    if (!empty($_POST['content_produit']) AND !empty($_POST['content_quantite']) AND !empty($_POST['ville']) AND !empty($_POST['quartier']) ) {
  $activite="vente";
  $activity_coach="coach vente";
  $id_coach='NULL';
  $idfacture="";
      ...
      $idfacture="CTL/TR/". $Date."/".$codefactvar+=1;
       ...
       $idfacture="CTL/TR/". $Date."/0001";
        ...
        foreach ($content_produit as $key=>$content_produit) {
            ...
            $main->query($sql,array(null,null,$content_quantite[$key],$_SESSION['idMission'],date("Y-m-d"),$_SESSION['matricule'],$date,$content_produit,$_POST['ville'],$_POST['quartier'],$equipe['designationEquipe'],$id_coach,$idfacture,$idPrix[$key]));
          }
 }
}
?>
